...
namespace BitPayAPI
{
    ...
    public class InvoiceParams
    {
        ...
        public Dictionary<string, decimal> payouts { get; set; }
        ...
        public Dictionary<string, string> getDictionary()
        {
            ...
            {
                int i = 0;
                foreach (var item in this.payouts)
                {
                    parameters.Add("payouts[" + i + "].address", item.Key);
                    parameters.Add("payouts[" + i + "].weight", item.Value.ToString());
                    i++;
                }
